# Milestone v1.0.0: converge-core Restoration
...
- Traits First approach — define traits in core, implementations stay temporarily, full extraction in v2
- Type-state pattern for Proposal: Draft (public) vs Validated (crate-only)
- Fact::new() is pub(crate) — external code cannot create Facts
- ValidationToken ZST for forgery prevention (no runtime cost)
- GAT async pattern for capability traits (ChatBackend, EmbedBackend, Recall, Store)
- Git-based baseline comparison for cargo-semver-checks (not crates.io)

**Issues Resolved:**

- Removed rayon, rand, sha2 dependencies (replaced with stub traits)
- Fixed 16 forbidden crate categories with cargo-deny
- Achieved 39/39 requirement verification with code evidence

**Issues Deferred:**

- 518 clippy warnings (pre-existing technical debt)
- cargo fmt formatting differences (pre-existing)
- Full implementation extraction to capability crates (v2 work)

**Technical Debt Incurred:**

- Existing implementations marked #[deprecated] but not removed (migration path for downstream)
- hex crate allowed for ContentHash serialization (non-cryptographic use)
...
**Coverage: 39/39 requirements verified**

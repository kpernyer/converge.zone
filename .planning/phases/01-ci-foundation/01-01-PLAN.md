...
This failure proves the deny.toml is working correctly - it will pass after Phase 2 removes these deps
  ...
  <done>
deny.toml exists with deny list containing all 13 forbidden crates (tokio, async-std, async-trait, reqwest, hyper, axum, rayon, rand, rand_core, sha2, hex, burn, polars, ndarray). Each entry has a reason field. cargo deny check runs and reports violations for the 4 currently-present forbidden deps.
  </done>
</task>
...
4. "## FORBIDDEN Dependencies" section with table:
   ...
   Categories: Async Runtime, HTTP/Network, Parallelism, Randomness, Hashing, ML/Data
   List all 13+ forbidden crates with reasons and alternatives
...
Contains "## FORBIDDEN Dependencies" section with all 13 forbidden crates
Contains "## Enforcement" section referencing deny.toml
  </verify>
  <done>
PURITY.md exists with ALLOWED table (5 crates with justifications), FORBIDDEN table (13+ crates with reasons/alternatives), Enforcement section referencing deny.toml and CI pipeline. Document is self-contained and explains the purity philosophy.
  </done>
</task>
...
4. [x] deny.toml covers all forbidden patterns from PROJECT.md
   - VERIFIED BY: Task 1 includes all 13 forbidden crates from PROJECT.md
</success_criteria>
